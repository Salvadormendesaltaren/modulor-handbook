# FIK

> Shopify Premier Partner - Desarrollo e-commerce

---

## 01. Identidad y Estrategia

### Datos básicos

- **Nombre:** FIK
- **Web:** https://fik.life
- **Fundación:** 2014
- **CEO:** Bill Murphy
- **COO:** Brian Van Andel
- **Sede:** Madrid
- **Empleados:** 6 personas
- **Certificación:** 🏆 **Shopify Premier Partner** (certificación más alta)

### Propósito

- **Misión:** Ayudar a equipos de e-commerce a lanzar más rápido y con menos riesgo combinando expertise en plataforma, ejecución omnicanal y disciplina de performance
- **Visión:** Ser el Shopify Premier Partner de referencia en España para marcas de moda y gran consumo, conocido por delivery fiable, profundidad de plataforma y resultados medibles, mientras expandimos por Europa
- **Especialización:** 100% foco en Shopify con +10 años de experiencia

### Posicionamiento

- **¿Qué somos?** Agencia especializada en Shopify Plus (top tier)
- **¿Para quién?** Marcas D2C y retailers que quieren vender online con Shopify
- **¿Qué nos hace diferentes?**
    - Premier Partner (certificación más alta de Shopify)
    - +10 años de experiencia exclusiva en Shopify
    - +42 proyectos Shopify Plus completados
    - Clientes que facturan +15M€ de media

### Cómo ganamos vs competidores

- **Governance + delivery reliability** (SLAs, QA, release discipline) en vez de solo "great design/dev"
- **Unified commerce credibility** (POS + integrations) + resultados de performance medibles
- **Retainer squads** con modelo comercial claro (no ad-hoc time & materials)
- **Modulor leverage** para ganar scopes más grandes end-to-end (creative + growth + tech)

### Métricas clave

| Métrica | Valor |
| --- | --- |
| Facturación media anual de clientes | 15M€ |
| Tiempo típico de migración | 12 semanas |
| Proyectos Shopify Plus | 42+ |

### Qué NO hacemos

- Otras plataformas (solo Shopify)
- Diseño de marca/branding (derivamos a mendesaltaren)
- Marketing y social media (derivamos a SSTIL)
- Desarrollo de software custom (derivamos a Tailor Hub)

---

## 02. Estructura y Equipo

### Leadership

| Nombre | Puesto |
|--------|--------|
| Bill Murphy | CEO |

### E-commerce

| Nombre | Puesto |
|--------|--------|
| Brian Van Andel | Shopify Expert |
| Pablo Pellico | Shopify Expert |

### Tecnología

| Nombre | Puesto |
|--------|--------|
| Jose Luis Cantalejo | Frontend Developer |
| Noemi Maroñas | Frontend Developer |
| Enrique Melero | Backend Developer |

**Total:** 6 personas

### Contrataciones previstas 2026

| Rol | Timing | Notas |
| --- | --- | --- |
| Shopify Expert | Q1 2026 | Refuerzo equipo core |
| Performance + Analytics | Q2 2026 | Para nuevo servicio de Performance |
| TBD (Delivery lead / AM / Senior engineer) | Q? 2026 | Según bottleneck |

---

## 03. Servicios

### Prioridades 2026 (del Business Plan)

1. **Retainer Squads** — Prioridad alta
2. **Performance (CRO + Analytics)** — Prioridad alta (nuevo/escalado)
3. **Migrations & Replatforms** — Core
4. **Integrations & Custom Dev** — Core
5. **Fractional CDO** — Nuevo servicio

---

### Retainer Squads (Roadmap Delivery) 🔥 PRIORIDAD

- **Descripción:** Bloques de capacidad mensual con cadencia de planning, demos y ownership de backlog
- **Incluye:** SLAs opcionales (tiempos de respuesta, monitoring, soporte de incidentes)
- **Modelo:** Retainer mensual

### Performance & Analytics 🔥 PRIORIDAD (Nuevo 2026)

- **Descripción:** Fundamentos de medición y programa de CRO
- **Incluye:** Tracking, dashboards, attribution readiness, CRO + experimentación

### Replatforming / Migraciónes

- **Descripción:** Migración completa de infraestructura e-commerce a Shopify
- **Desde:** Magento, WooCommerce, PrestaShop, Salesforce Commerce, etc.
- **Incluye:** Análisis de negocio, planificación, ejecución, migración de datos
- **Duración típica:** 12 semanas

### Theme Development

- **Descripción:** Desarrollo de temas custom para Shopify
- **Incluye:** E-commerce branded, herramientas para marketing, Themes 2.0

### UX/UI Design

- **Descripción:** Auditoría y diseño de experiencia e-commerce
- **Incluye:** Auditoría, definición de producto, concepto visual, diseño de interfaces

### Payments & Checkout

- **Descripción:** Desarrollo de métodos de pago y personalización de checkout
- **Certificación:** Payment Provider Partner
- **Incluye:** Checkout Extensibility, integración de pasarelas

### E-commerce Consultancy

- **Descripción:** Optimización de operaciónes y logística e-commerce
- **Incluye:** Selección de stack tecnológico, colaboración con partners

### Platform Integrations

- **Descripción:** Conectores con sistemas empresariales
- **ERP:** SAP, Navision, Business Central, NetSuite, Odoo
- **CRM:** Salesforce
- **Marketplaces:** Integraciónes multi-canal

### Analytics & Data

- **Descripción:** Tracking y análisis de datos e-commerce
- **Incluye:** GA4, GTM, analytics personalizados, dashboards integrados

### Shopify POS

- **Descripción:** Estrategia omnicanal con punto de venta físico
- **Incluye:** Migraciónes de POS, consultoría POS, estrategia omnicanal

### Shopify Plus Training

- **Descripción:** Formación para equipos internos
- **Incluye:** Documentación, videos, Shopify Flow, Launchpad, recruiting

### Fractional CDO (Nuevo 2026)

- **Descripción:** Commerce roadmap + governance + coordinación de vendors
- **Incluye:** KPI framework, priorización, oversight de delivery entre stakeholders
- **Modelo:** Servicio empaquetado

### Productos (Apps)

- **Pickup Points** — App de puntos de recogida
- **Store Credit Manager** — App de gestión de crédito en tienda

---

## 04. Modelo de Negocio

### Planes de Retainer (Modelo actual)

> ⚠️ Este modelo está en proceso de cambio hacia Retainer Squads (ver Prioridades 2026)

| Plan | Fee mensual | Incluye |
| --- | --- | --- |
| **Basic** | €100/mes | Equipo experto Shopify Plus, Asana, 30min reunión mensual, hasta 1 tienda POS |
| **Starter** | €250/mes | + Soporte incidencias urgentes, Analítica, reporting mensual, Notion, 4h reuniones/mes, hasta 4 tiendas POS |
| **Advance** ⭐ | €550/mes | + Customer Success Manager, reuniones semanales, Slack/email/teléfono, 11h reuniones/mes, hasta 8 tiendas POS |

### Tarifas por Hora según Plan

| Perfil | Basic | Starter | Advance |
| --- | --- | --- | --- |
| Shopify Expert | €100/h | €90/h | €80/h |
| Shopify Front-end | €100/h | €90/h | €80/h |
| Back-end | €150/h | €135/h | €120/h |
| Analítica y SEO | €140/h | €125/h | €110/h |
| Integration Architect | €160/h | €140/h | €125/h |

**Modelo de facturación:**
- Fee mensual = acceso al servicio
- Horas de trabajo = facturadas aparte según perfil y plan
- Sprints de 2 semanas, gestión en Asana
- Estimaciones aprobadas por cliente antes de ejecutar

### Objetivos 2026

- **Facturación 2025:** €450k
- **Objetivo 2026:** €700k (+25%)
- **MRR objetivo:** 40% recurrente
- **Margen objetivo:** 10%

### Go-To-Market 2026

| Canal | % | Descripción |
| --- | --- | --- |
| Shopify channel | 50% | Co-selling, ecosystem credibility, case-study driven trust |
| Modulor cross-sell | 35% | Joint accounts, bundled pitches, shared pipeline |
| Inbound | 5% | Proof content (case studies, playbooks, performance outcomes) |
| Outbound | 5% | Targeted Italy + select strategic accounts |
| Network | 5% | Founder-led pero lightweight |

**Expansión Italia 2026:**
- BD builds pipeline vía Shopify ecosystem, partner referrals, outbound targeted
- Goal: 1-3 primeros clientes italianos (mix de audits → squad retainers)

---

## 05. Cliente Ideal (ICP)

### Perfil de empresa

**Primary:** Fashion y grandes marcas de consumo con complejidad operativa:
- Multi-mercado, alto SKU, picos estacionales, footprint retail, necesidades POS
- Equipos que necesitan estabilidad + ejecución de roadmap (no proyectos one-off)

**Secondary:** Marcas financiadas acercándose a escala que requieren disciplina de plataforma

- **Sectores:** Fashion, Beauty, Lifestyle, DTC brands, Retail
- **Geografía:** España, Europa (expansión Italia 2026)
- **Facturación e-commerce mínima:** >1M€
- **Presupuesto mínimo:** >30k€

### Perfil del decisor

- **Cargo típico:** CEO, CMO, E-commerce Manager, Director Digital
- **Motivaciónes:** Escalar ventas online, mejorar conversión, internacionalizar
- **Frustraciones:** Plataforma actual limitante, operaciónes manuales, falta de omnicanalidad

### Triggers de compra

- Lanzamiento de marca D2C
- Migración desde Magento/WooCommerce/PrestaShop
- Crecimiento que requiere Shopify Plus
- Rediseño/rebranding de tienda
- Expansión internacional
- Apertura de canal retail (POS)

### Señales de buen fit

**🟢 Green flags:**
- Marca con identidad clara
- Facturación e-commerce >5M€ (o proyección clara)
- Presupuesto adecuado
- Entienden el valor de Shopify Plus

**🔴 Red flags:**
- Presupuesto muy limitado
- Expectativas de "tienda gratis"
- Sin claridad en producto/marca
- Quieren otra plataforma

---

## 06. Casos y Portfolio

### Casos destacados

#### Ecoalf
- **Contexto:** Marca de moda sostenible
- **Reto:** Migración desde Prestashop a Shopify Plus
- **Qué hicimos:** Migración completa, integración SAP, POS internacional
- **Servicios:** Replatforming, Integrations, POS

#### Cristina Oria
- **Contexto:** Marca gastronomía premium
- **Reto:** Migración desde Magento
- **Qué hicimos:** Migración a Shopify, automatización logística
- **Servicios:** Replatforming

#### Bimani
- **Contexto:** Marca de moda
- **Reto:** Migración completa en tiempo récord
- **Qué hicimos:** Migración a Shopify y Shopify POS en 14 semanas
- **Servicios:** Replatforming, POS

#### Polo Club
- **Contexto:** Marca de moda lifestyle
- **Qué hicimos:** Desarrollo de tema custom con Themes 2.0
- **Servicios:** Theme Development

#### Utópica Travel (El Corte Inglés)
- **Contexto:** E-commerce de viajes de lujo
- **Qué hicimos:** Desarrollo con integración Salesforce CRM
- **Servicios:** Theme Development, Integrations

#### Suot Studio (Marta Tous)
- **Contexto:** Marca de joyería
- **Qué hicimos:** Tema custom con configurador de producto, B2B
- **Servicios:** Theme Development

### Clientes destacados

Telefónica, El Corte Inglés, Sony Music, Bimani, Milbby, Hoff, Mas'cob, Nanimarquina, Cristina Oria, TOUS (Suot Studio), La Agam, Ecoalf, Mietis, Polo Club, Singularu, Bobo Choses

### Métricas

- **Tiendas lanzadas:** 150+
- **Proyectos Shopify Plus:** 30+
- **Revenue gestiónado por clientes:** +300M€

### Partners tecnológicos

Klaviyo, Yotpo, Narvar, Sendcloud, ReturnGo, Moddo, Nosto, Klarna, Synerise, Reveni

---

## 07. Cross-selling y Sinergias

### Cuándo derivar A FIK

| Señal | Desde |
| --- | --- |
| Cliente quiere e-commerce | Cualquiera |
| Cliente menciona Shopify | Cualquiera |
| Marca D2C necesita tienda | mendesaltaren, SSTIL |
| Migración desde otra plataforma | Cualquiera |

### Cuándo derivar DESDE FIK

| Señal | Hacia |
| --- | --- |
| Cliente necesita branding/diseño UX | mendesaltaren |
| Integraciónes muy complejas | Tailor Hub |
| Cliente necesita marketing/social | SSTIL/Minimum |
| Automatizaciones operativas | Nocodehackers |

### Proyectos conjuntos típicos

- **FIK + mendesaltaren:** Tienda Shopify + branding/UX premium
- **FIK + SSTIL:** Tienda + marketing de lanzamiento
- **FIK + Nocodehackers:** Tienda + automatizaciones/integraciónes
- **FIK + Tailor Hub:** Tienda + backend complejo

---

## 08. Ventas

### Mensajes clave

- **Elevator pitch:** Somos Shopify Premier Partner con +10 años de experiencia exclusiva en Shopify. Hemos lanzado +150 tiendas que gestiónan +300M€ en revenue. Trabajamos con marcas como El Corte Inglés, Ecoalf y Singularu.
- **Diferenciación:** Premier Partner (top tier), +42 proyectos Shopify Plus, clientes que facturan 15M€ de media

---

*Última actualización: Febrero 2026*
