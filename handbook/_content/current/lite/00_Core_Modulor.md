# Modulor Studios - Core

> Documento central del grupo. Contexto mínimo para operar a nivel transversal.

---

## 01. Identidad del Grupo

### Datos básicos

- **Nombre:** Modulor Studios
- **Estructura:** Holding / Grupo de empresas
- **Sede central:** Calle Hermanos Álvarez Quintero, 2, 28004 Madrid
- **Web corporativa:** https://www.modulorstudios.com
- **Total Grupo:** 125 personas (+ 7 en Modulor central)

### Visión

Redefinir cómo las organizaciónes más ambiciosas del mundo diseñan, construyen y crecen, liderando el progreso creativo y tecnológico.

### Misión

Conectamos ideas, personas y tecnología a través de estudios especializados para construir soluciones que hacen avanzar a las organizaciónes. Creemos que el mejor trabajo ocurre cuando estrategia, creatividad y tecnología se construyen juntas — no por separado. Trabajamos codo a codo con las organizaciónes que están dando forma al mañana, convirtiendo innovación en producto, marca y experiencia.

### Estrategia

Construir un holding consolidado que dé a cada boutique las herramientas, la tecnología y la escala que normalmente solo está al alcance de las grandes consultoras, sin sacrificar lo que nos hace diferentes: equipos pequeños con criterio, autonomía y oficio. El resultado: capacidad real para competir por proyectos enterprise y presupuestos que una boutique sola no puede alcanzar. The AI Native Agency.

### Modelo de relación

De "conectados, no acoplados" a "cohesionados y enérgicos". El grupo evoluciona de un modelo de coordinación ligera (pensado para diversificación) a un modelo de cohesión real que permita abordar grandes apuestas juntos — sin perder la identidad y autonomía de cada estudio.

### Elevator pitch

Modulor Studios es un grupo de boutiques especializadas en diseño, tecnología, marketing y formación digital. Ofrecemos soluciones integrales desde diseño de producto hasta desarrollo de software, e-commerce y marketing.

---

## 02. Los 6 Pilares Estratégicos

| # | Pilar | Descripción |
| --- | --- | --- |
| 1 | **Crecimiento orgánico** | Cross-selling, upselling, expansión de cuentas |
| 2 | **Crecimiento inorgánico** | M&A, adquisición de nuevos estudios |
| 3 | **Expansión internacional** | Nuevos mercados, nuevas geografías |
| 4 | **Modulor OS (Tecnología e IA)** | Sistema operativo de grupo: herramientas unificadas, procesos cross-boutique, IA integrada en operaciones |
| 5 | **Posicionamiento y comunicación** | Arquitectura de marca, thought leadership, visibilidad en el mercado |
| 6 | **Personas y liderazgo** | Talento, cultura, desarrollo del liderazgo |

---

## 03. Mapa de Boutiques

| Boutique | Especialización | CEO | Headcount | Web |
| --- | --- | --- | --- | --- |
| **mendesaltaren** | Diseño digital y branding | Sara Rivera | 50 | mendesaltaren.com |
| **Tailor Hub** | Desarrollo software, IA y Mobile Apps | Diego Méndez | 31 | tailor-hub.com |
| **FIK** | Desarrollo Shopify (Premier Partner) | Bill Murphy | 6 | fik.life |
| **SSTIL** | Marketing, comunicación y growth (submarca: Minimum) | Paula Rodríguez + Sandra Ramiro | 19 | sstilagency.com |
| **Nocodehackers** | Formación No-code/IA + Agencia | Alex Vaughtton | 10 | nocodehackers.es |

### Especialización por boutique

| Capacidad | Empresa(s) | Notas |
| --- | --- | --- |
| Diseño de producto | mendesaltaren | Enterprise |
| Branding | mendesaltaren | Estratégico, orientado a producto |
| Design Systems | mendesaltaren | |
| Desarrollo software enterprise | Tailor Hub | Backend, apps complejas |
| IA/ML avanzado | Tailor Hub | LLMs, modelos custom |
| Mobile Apps Development | Tailor Hub | Nativo y cross-platform |
| E-commerce Shopify | FIK | Premier Partner, Shopify Plus |
| Marketing/Social | SSTIL | Marcas establecidas |
| Growth/Webs rápidas | SSTIL (Minimum) | Startups, Webflow |
| Automatizaciones | Nocodehackers | Make, n8n, Zapier |
| MVPs/Apps no-code | Nocodehackers | Bubble, FlutterFlow |
| Formación no-code/IA | Nocodehackers | +10k alumnos |

---

## 04. Governance y Liderazgo

### Leadership de boutiques

| Boutique | Rol | Persona |
| --- | --- | --- |
| mendesaltaren | CEO | Sara Rivera |
| mendesaltaren | Head of People | Patricia Pérez |
| mendesaltaren | Head of Product & Data Lead | Fani Pérez |
| mendesaltaren | Head of Operations | Carlos Sánchez |
| mendesaltaren | Head of Communications | Paloma García Almodovar |
| mendesaltaren | Business Developer | Paula López |
| Tailor Hub | CEO | Diego Méndez |
| Tailor Hub | CCO | Simón G. Negrete |
| Tailor Hub | Head of Engineering | Nikolaas Verlee |
| Tailor Hub | Operations & Project Manager | Carolina Velo |
| Tailor Hub | Senior Project Manager | Víctor Rodríguez |
| FIK | CEO | Bill Murphy |
| FIK | COO | Brian Van Andel |
| SSTIL | Co-CEO | Paula Rodríguez |
| SSTIL | Co-CEO | Sandra Ramiro |
| SSTIL | Sales Manager | Emiliano Moretti |
| SSTIL | Project Manager | Mia Escorihuela |
| SSTIL | Lead Designer | Alba Rodríguez |
| Nocodehackers | CEO | Alex Vaughtton |
| Nocodehackers | CMO | Tomás Alonso |
| Nocodehackers | Ops | Stefano Becerra |

### Roles corporativos (Modulor central)

| Rol | Persona | Funciones |
| --- | --- | --- |
| CEO | Jorge Lana | Visión, dirección general |
| COO/CPO | Salva Serrano | Operaciones del grupo, coordinación entre boutiques, procesos compartidos y colaboración intragrupo. Creación de producto interno, transformación IA, implementación de herramientas y nuevos procesos coordinando con CTO |
| CCO | André Mendes | Creatividad, experiencia |
| CFO | Viviana Reyes | Finanzas |
| People | Carina Matyniak | HR, Office Management |
| Legal | Legal Army (partner) | Asesoría legal |

### Equipo Comercial del Grupo

| Boutique | Persona | Rol | Foco |
|----------|---------|-----|------|
| **Modulor** | Jorge Lana | CEO | Dirección estratégica del grupo, Business Plan, M&A, relaciones con partners estratégicos y clientes, representación institucional, ventas transversales, supervisión del leadership team |
| **Modulor** | André Mendes | Socio / CCO | Posicionamiento estratégico del grupo, dirección creativa transversal, supervisión de las prácticas de diseño, cultura y experiencia del grupo y cliente, apoyo con Business Plan y supervisión leadership team |
| **Modulor** | Salva Serrano | COO/CPO | Operaciones, producto, colaboración |
| **mendesaltaren** | Sara Rivera | CEO | Clientes clave, estratégico |
| **mendesaltaren** | Paula López | Business Developer | Captación y desarrollo |
| **Tailor Hub** | Diego Méndez | CEO | Internacional |
| **Tailor Hub** | Simón G. Negrete | CCO | Nacional |
| **Tailor Hub** | Alex Alegria | Business Developer | Captación |
| **SSTIL** | Paula Rodriguez | Co-CEO | Estrategia, desarrollo negocio |
| **SSTIL** | Sandra Ramiro | Co-CEO | Creatividad, relaciones |
| **SSTIL** | Emiliano Moretti | Sales Manager | Comercial |
| **FIK** | Bill Murphy | CEO | Founder-led |
| **FIK** | Brian Van Andel | COO | Comercial + delivery |
| **Nocodehackers** | Alex Vaughtton | CEO | Founder-led |

---

## 05. Departamentos Modulor

### Modulor OS

Modulor OS es el sistema operativo del grupo: el marco que suma tecnología, procesos y automatizaciones para que cada boutique crezca de forma eficiente sin perder lo que la hace diferente.

**Los 8 pilares de valor para los estudios:**

| # | Qué pedimos | Qué consigues |
|---|---|---|
| 1 | Alineación con herramientas comunes | El mejor en tu disciplina |
| 2 | Adopción de procesos unificados | Gobierno del dato y herramientas comunes |
| 3 | Participación en dinámicas de grupo | Colaboración intragrupo |
| 4 | Apertura a transformación IA interna | Transformación IA interna |
| 5 | Contribución a la cultura compartida | Cultura única y diferenciadora |
| 6 | Reporting y cadencias | Operaciones compartidas |
| 7 | Derivación activa de leads | Cross-sell con impacto real |
| 8 | Eficiencias en delivery | Eficiencia operativa y márgenes mayores |

**Lo que vamos a cambiar:**
- Unificar herramientas comunes (Workspace 2.0)
- Unificar procesos operativos cross-boutique
- Alinear liturgias, incentivos e impulsar cultura compartida

**Lo que no podemos perder:**
- Identidad por boutique
- Capacidad de innovar en servicio
- Cultura propia ligada a disciplina
- Ownership por boutique

### OS de Proyectos

Estructura organizativa para proyectos multidisciplinares grandes (>2M€) que involucran a varias boutiques:

**Dirección:**
- Project Director — Visión y responsabilidad del proyecto
- Account Manager — Relación con cliente y gestión comercial

**Leads por disciplina:**

| Rol | Foco |
|---|---|
| Strategy Lead | Estrategia y business design |
| Digital Design / Digital Experience Lead | Diseño de producto y experiencia |
| Tech Lead | Arquitectura, desarrollo, IA |
| Project Manager | Gestión operativa del proyecto |
| Product Owner | Backlog, priorización, roadmap |
| MKT / Distribution Lead | Marketing, lanzamiento, growth |
| Physical Experience / Strategist | Experiencia física, eventos |

Cada lead coordina especialistas de la boutique correspondiente.

### Proyectos Estratégicos

| Proyecto | Descripción | Status |
|---|---|---|
| **Agente de ventas / Content Factory** | Agente de IA que genera propuestas comerciales, presentaciones y contenido a partir del contexto del handbook y datos del grupo | En desarrollo |
| **Deal Registry / Automatización de contratos, proyectos y colaboración** | Pipeline automatizado desde el registro de oportunidades hasta la generación de contratos y el kickoff de proyectos cross-boutique | En desarrollo |
| **OS de Proyectos** | Estructura organizativa para proyectos multidisciplinares grandes (ver sección arriba) | Definido |
| **Workspace 2.0** | Unificación del stack de herramientas bajo dominio común (ver Herramientas §10) | En curso |

---

### Ops (Operaciones)

**Logros:**
- Fichar perfil Modulor Ops
- Auditoría y plan de seguridad
- Soporte integración FIK
- Proyecto creación de HubSpot realizado sin contar con partner hasta el final
- Primer bottleneck identificado: Deal-to-kickoff

**Dificultades:**
- Medir el ROI de lo que hacemos
- Sacar tiempo de lo táctico para resolver los grandes bloqueos
- Fichar un perfil muy escaso
- Extraer la información de múltiples fuentes

**Próximos pasos:**
- OnBoarding perfil y Offboarding Senda
- Configuración Senda
- Resolver primer bottleneck: Deal-to-kickoff
- Medir el ROI de Modulor Ops
- Conseguir más recursos para el departamento
- Mejorar la comunicación y la colaboración dentro del grupo

---

### Tech (Tecnología e IA)

Modulor Tech es la iniciativa para integrar IA de forma estructural en las operaciónes del grupo.

**Logros:**
- Diagnóstico claro y compartido: Problema no es tecnológico, es estructural
- Identificación de quick wins concretos
- Enfoque por fases definido

**Dificultades:**
- Cultura de comunicación informal: Si no entra por el canal oficial, el sistema no lo puede mejorar
- Falta de procesos cross-boutique formales
- Automatizar sin proceso definido amplifica el caos
- Calidad y estandarización de datos (CRM)
- Capacidad y foco de los equipos

**Roadmap por fases:**

| Fase | Timing | Foco | Medición |
| --- | --- | --- | --- |
| **Fase 1: Foundations** | Q1 | Pilotos de automatización: Deal registry (Pipedrive→Notion), contratos freelance (Tally→Doc), tickets de comunicación | Tiempo de setup, errores manuales eliminados |
| **Fase 2: Integration** | Q2 | Conectar flujos: Operations + Finance + Communications en pipeline unificado (Won→Contract→Kickoff) | Tiempo deal-to-delivery, adopción entre equipos |
| **Fase 3: AI Native** | Q3/Q4 | Sistemas inteligentes: Knowledge base consultable por IA, agente de presentaciones comerciales auto-generadas | Calidad de outputs, autonomía del sistema, time-to-insight |

---

### Finance

**Dificultades/Retos:**
- Escalar sin aumentar costes operativos y aumentar márgenes
- Negocios más predecibles (↑ MRR) - Mínimo 70% en cada boutique
- Asegurar estabilidad financiera por boutique (Reto: FIK y TH)
- Compliance: Homologaciones IA Ethics, Cybersecurity, ISOs

**Próximos pasos:**
- Simplificar y unificar procesos apoyo (HHRR, Finance, admin) respetando el ADN de cada boutique
- Buscar cómo ganar eficiencia: quitar trabajo manual, apoyarnos en tecnología (automatizaciones, IA)
- Foco en el crecimiento y la rentabilidad objetivo de este año

---

### Culture (Personas y Cultura)

**Logros:**
- De 5 empresas sueltas a 1 grupo
- Mallorca como punto de partida
- Cada líder del grupo tiene ahora un espacio propio para crecer
- Sabemos quiénes somos, qué nos mueve y cómo decidimos juntos

**Dificultades:**
- Aún no medimos lo que importa (OKRs pendientes)
- No tenemos un idioma común para vender, pero ya se empieza a sentir una cultura de ventas más fuerte (Handbook de ventas)
- Sistema de gobernanza is coming

**Próximos pasos:**
- Convertir a cada manager en coach
- Ibiza Retreat
- Que cada persona sepa hacia dónde puede crecer (Career paths)
- Que cada boutique aprenda de la mejor versión de las demás

---

### Equipo de Comunicación Modulor

Función transversal de comunicación del grupo, con perfiles de distintas boutiques:

| Nombre | Boutique base | Rol |
|---|---|---|
| Paloma Almodóvar | mendesaltaren | Head of Communications |
| Bia Ferreira | mendesaltaren | Comms |
| Adrián Beloki | Tailor Hub | Comms |
| André Mendes | Modulor Central | CCO |

### Comunicación

**Logros:**
- Modulor tiene voz propia por primera vez
- Rediseño de la web del grupo
- Summit de Invierno como cambio en el nivel de calidad de un evento organizado por el grupo
- Equipo definido con perfiles de varias empresas del grupo con roles y dedicaciones claras
- Plan de comunicación 2026

**Dificultades:**
- Proceso manual para la administración de solicitudes
- Cadencia de contenido irregular
- No está claro cuándo habla Modulor y cuándo hablan las boutiques
- Narrativa genérica: aún no sabemos decir qué nos diferencia de una consultora
- Mercados internacionales identificados pero sin priorizar
- Nadie que sepa de front en el equipo

**Próximos pasos:**
- Consolidar el Sistema operativo de Comms (peticiones, calendario, triage)
- SSTIL como agencia interna de marketing
- Definir arquitectura de voz
- Workshop de narrativa con Partners para cerrar diferenciación
- LinkedIn como canal principal, cadencia estable
- Priorizar mercados internacionales con Sales
- PR y visibilidad internacional en segundo semestre

---

## 06. Modelo Financiero del Grupo

### Track Record (2020-2025)

| Métrica | Dato |
| --- | --- |
| Crecimiento 2020-2025 | €1M → €9M (54% CAGR) |
| Revenue recurrente | 60%+ |
| EBITDA | 10-12% |
| Presupuesto de inversión | Autofinanciado |
| Presencia internacional | 10+ países |

### Objetivos Financieros

| Año | Revenue | EBITDA | Notas |
| --- | --- | --- | --- |
| 2025 | €9M (base) | 10-12% | Año actual |
| 2026 | €12.3M-12.5M | 15% | +30% grupo, +25% mínimo por estudio |
| 2027 | €16M | 10-12% | Nueva adquisición |
| 2028 | €22M | 10-12% | Productos + M&A |

### Objetivos por Boutique 2026

| Boutique | Revenue 2025 | Objetivo 2026 | Growth |
| --- | --- | --- | --- |
| mendesaltaren | €4.6M | €5.5M | +20% |
| Tailor Hub | €2.8M | €3.8M | +36% |
| Nocodehackers | €450k | €800k | +78% |
| FIK | €550k | €700k | +27% |
| SSTIL/Minimum | €1.3M | €1.7M | +31% |

**Facturación reservada grupo:** €8.1M (66% del objetivo)

### Resultados Q1 2026

| Métrica | Resultado |
|---|---|
| Facturación Q1 | €3M (96% del objetivo trimestral) |
| Rentabilidad | 14% |
| Crecimiento YoY | +30% (vs Q1 2025: €2.3M, 11% margen) |

| Boutique | Q1 Y26 | % Objetivo |
|---|---|---|
| mendesaltaren | €1.3M | ~98% |
| Tailor Hub | €836k | ~85% |
| SSTIL | €468k | ~97% |
| Nocodehackers | €255k | 100%+ |
| FIK | €175k | 100%+ |

---

## 07. Colaboración entre Boutiques

### Evolución del paradigma de colaboración

**AS-IS: Conectados, no acoplados.** Boutiques coordinadas pero independientes. Cada estudio mantiene su core, su equipo y su identidad. Modulor conecta pero no integra. Un paradigma pensado para la diversificación — pero que limita la capacidad del grupo para abordar grandes proyectos de forma conjunta.

**TO-BE: Cohesionados y enérgicos.** Un grupo unido con capacidad real de abordar grandes apuestas que ninguna boutique podría asumir sola. Procesos, herramientas y cultura compartida que permiten movilizar talento multidisciplinar de forma ágil — sin perder la identidad y ownership de cada estudio.

### Matriz de Derivación de Leads

| Si el lead necesita... | Derivar a... |
| --- | --- |
| Diseño de producto digital | **mendesaltaren** |
| Branding de producto o gran escala | **mendesaltaren** |
| Design systems | **mendesaltaren** |
| Desarrollo software enterprise | **Tailor Hub** |
| IA/ML avanzado | **Tailor Hub** |
| Mobile Apps (nativo/cross-platform) | **Tailor Hub** |
| E-commerce Shopify | **FIK** |
| Migración a Shopify Plus | **FIK** |
| Marketing/Social media | **SSTIL** |
| Activaciónes/Eventos | **SSTIL** |
| Growth marketing | **SSTIL** (Minimum) |
| Webs rápidas (Webflow) | **SSTIL** (Minimum) |
| Automatizaciones | **Nocodehackers** |
| MVPs/Prototipos | **Nocodehackers** |
| Apps no-code | **Nocodehackers** |
| Formación no-code/IA | **Nocodehackers** |

---

## 08. Combinaciones Ganadoras

### Lab 226: Innovation + Strategy + Technical Audit

1. **mendesaltaren** → Visión estratégica, business design, co-creation labs
2. **Tailor Hub** → Technical audits (arquitectura, datos, sistemas), AI-readiness
3. **Nocodehackers** → Prototipado funciónal, MVPs rápidos, interfaces ágiles
4. **Resultado** → Roadmaps accionables para transformación AI-driven

### Proyecto integral de producto digital

1. **mendesaltaren** → Estrategia, diseño de producto, branding
2. **Tailor Hub** → Desarrollo de software
3. **SSTIL** (Minimum) → Lanzamiento y growth

### Proyecto e-commerce completo

1. **mendesaltaren** → Diseño de experiencia y branding
2. **FIK** → Desarrollo Shopify Plus
3. **Nocodehackers** → Automatizaciones operativas
4. **SSTIL** → Comunicación y social media

### Startup/MVP rápido

1. **Nocodehackers** → MVP y validación
2. **mendesaltaren** → Diseño cuando escale
3. **Tailor Hub** → Desarrollo cuando necesite código

### Transformación digital corporativa

1. **mendesaltaren** → Design systems y producto
2. **Tailor Hub** → Desarrollo y arquitectura
3. **Nocodehackers** → Automatizaciones internas
4. **SSTIL** → Comunicación del cambio

### Casos reales de colaboración intragrupo (Q1 2026)

15 proyectos cross-boutique activos en Q1 2026:

| Proyecto | Boutiques |
|---|---|
| Novo Nordisk | Tailor Hub + Nocodehackers |
| Volare | mendesaltaren + Tailor Hub |
| PayPal | mendesaltaren + Tailor Hub |
| Mundi Ventures | mendesaltaren + Tailor Hub |
| Clínica Bratos | mendesaltaren + Nocodehackers |
| Orbegozo | mendesaltaren + SSTIL |
| Essentialist | mendesaltaren + SSTIL |
| Sillia | mendesaltaren + Tailor Hub |
| McKinsey + Orange | mendesaltaren + Tailor Hub |
| Base 4 | mendesaltaren + SSTIL |
| World Infras Forum | mendesaltaren + SSTIL |
| Diamond Foundry | mendesaltaren + FIK |
| Inditex | mendesaltaren + Tailor Hub |
| Seedtag | mendesaltaren + Tailor Hub |

---

## 09. Criterios de Cualificación Compartidos

### Budget

- Proyectos >30k€ para mendesaltaren, Tailor Hub y FIK
- Proyectos >10k€ para SSTIL y Nocodehackers
- Retainers mensuales >3k€

### Timing

- Proyectos con al menos 6-8 semanas de timeline
- No urgencias extremas sin presupuesto acorde

### Decisor

- Acceso directo a C-level o responsable de presupuesto
- Capacidad de tomar decisiones sin múltiples capas de aprobación

### Fit cultural

- Empresas que valoran la calidad sobre la velocidad extrema
- Organizaciones abiertas a recomendaciones estratégicas
- Clientes que entienden el valor del diseño/tecnología

---

## 10. Herramientas de Grupo (Workspace 2.0)

**Modulor Tools Unificadas** — Usadas por todas las boutiques bajo dominio común:

| Sistema | Para qué |
|---|---|
| Google Workspace | Email, docs, calendar (dominio único con alias por boutique) |
| Slack | Comunicación interna |
| Notion | Wiki, documentación, colaboración |
| HubSpot | CRM, ventas de grupo |
| Granola | Notas de reuniones con IA |
| Calendar | Planificación compartida |

**Studio Shared Tools** — Herramientas compartidas entre estudios:

| Sistema | Para qué |
|---|---|
| Figma | Diseño |
| Adobe | Suite creativa |
| Linear | Gestión de proyectos técnicos |
| Dropbox | Almacenamiento de archivos |
| Holded | Gestión administrativa/contable |
| NordLayer | VPN y seguridad de red |

**Studio Specific Tools** — Herramientas propias de cada boutique según su disciplina.

ISO27001 viable con esta configuración de workspace.

---

## 11. Clientes del Grupo

### Enterprise / Corporate

McKinsey & Company, El Corte Inglés, Iberia, Cargill, Inditex, Zara, Mondelez, WIO, Naturgy, Dormakaba, Mediapro, Repsol, Loewe, On Running, Novo Nordisk, Bilbao Museoa, Santander, Caixa Geral de Depósitos, Louis Vuitton, Orange, PayPal, Volare, Persepolis, Mundi Ventures

### Por sector

- **Energy and Industry:** Repsol, Naturgy, Smarkia, Sweno (ECI), Ekhi, Finanzauto, Titan Cement, Cyclica, Deiser
- **Financial Services:** Unicaja, Bnext, LDA, WioBank, Caixa, Bit2Me, Kfund, Lifex, Rhea, ioBuilders, Abacum
- **Sports:** Playtomic, Golfmanager, Dribble, ON Running
- **Fashion & Retail:** ZARA, Inditex, Loewe, El Corte Inglés, Mango, Mondelez, Worten, Mercadona, Decathlon, VRAI
- **Logistics:** Cargill, Mammoth, Obuu
- **Travel:** Intermundial, Oakmond, Es.Moli, IAG, Iberia, IATI Seguros
- **Health, Pharma and Science:** Idoctus, Novo Nordisk, Gameto, Overture, Rhea, Moggie
- **Security & Access:** dormakaba, Movistar Prosegur Alarmas
- **Media & Entertainment:** El confidencial, Newtral, MediaPro, RTVE, EITB, Seedtag

---

*Última actualización: Mayo 2026*
